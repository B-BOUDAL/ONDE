<main class=" pb-16 lg:pt-16 lg:pb-24 bg-gray-100 dark:bg-gray-900 antialiased">
  ...
  <div class="flex justify-between px-4 mx-auto max-w-screen-xl ">
      <article class="mx-auto w-full max-w-2xl format format-sm sm:format-base lg:format-lg format-blue dark:format-invert">
          <header class="mb-4 lg:mb-6 not-format">
              <address class="flex items-center mb-6 not-italic">
                  <div class="inline-flex items-center mr-3 text-sm text-gray-900 dark:text-white">
                      <img class="mr-4 w-16 h-16 rounded-full" src="https://www.iconpacks.net/icons/2/free-user-icon-3296-thumb.png" alt="Jese Leos">
                      <div>
                          <a href="#" rel="author" class="text-xl font-bold text-gray-900 dark:text-white">{{ $blog->author }}</a>
                          <p class="text-base text-gray-500 dark:text-gray-400">{{ $blog->date }}</p>
                      </div>
                  </div>
              </address>
          ...
          <figure><img src="{{asset('storage/blog').'/'. $blog->image }}" alt="">
          <h4 class="mb-4 text-3xl font-extrabold leading-tight text-gray-900 lg:mb-6 lg:text-4xl dark:text-white">{{ $blog->title }}</h4>
      ...
      <div class="grid gap-12 sm:grid-cols-2 lg:grid-cols-4">
        @foreach ($blogs as $blog)
          <article class="max-w-xs">
              <a href="#">
                  <img src="{{asset('storage/blog').'/'. $blog->image }}" class="mb-5 rounded-lg" alt="Image 1">
              </a>
